...
public class StructureFinder {
    ...
    /**
     * Find the nearest structure of a specific type using NMS.
     * This is equivalent to /minecraft:locate.
     * Used for /sg find command.
     * Supports both Mojang mappings (Paper/NeoForge) and intermediary mappings (Fabric).
     */
    public Location findNearest(World world, Location center, String structureType, int maxRadius) {
        try {
            // Initialize reflection cache if needed
            if (!reflectionCacheInitialized || cachedWorld != world) {
                if (!initReflectionCache(world)) {
                    plugin.getConfigManager().debug("Failed to initialize reflection cache for findNearest");
                    return null;
                }
            }
            
            // Parse ResourceLocation - try multiple class names
            ...
            if (resourceLocationClass == null) {
                plugin.getConfigManager().debug("Could not find ResourceLocation class");
                return null;
            }
            
            Object resourceLocation = parseResourceLocation(structureType, resourceLocationClass);
            if (resourceLocation == null) {
                plugin.getConfigManager().debug("Could not parse ResourceLocation for: " + structureType);
                return null;
            }
            
            // Get structure from the cached registry
            if (cachedStructureRegistry == null) {
                plugin.getConfigManager().debug("No cached structure registry");
                return null;
            }
            
            Method getMethod = findMethod(cachedStructureRegistry.getClass(),
                new String[]{"get", "method_10223", "m_7745_", "a"},
                resourceLocationClass);
            if (getMethod == null) {
                plugin.getConfigManager().debug("Could not find registry get method");
                return null;
            }
            Object structure = getMethod.invoke(cachedStructureRegistry, resourceLocation);
            
            if (structure == null) {
                plugin.getConfigManager().debug("Structure not in registry: " + structureType);
                return null;
            }
            
            // Create BlockPos using cached constructor
            ...
            Object structureHolder = wrapAsHolderMethod.invoke(cachedStructureRegistry, structure);
            
            Class<?> holderSetClass = findClass(
                "net.minecraft.core.HolderSet",
                "net.minecraft.class_6885",
                "net.minecraft.core.HolderSet$a"
            );
            if (holderSetClass == null) {
                plugin.getConfigManager().debug("Could not find HolderSet class");
                return null;
            }
            
            List<Object> holderList = new ArrayList<>();
            holderList.add(structureHolder);
            Method directMethod = findMethodByNames(holderSetClass, "direct", "method_40331", "m_203661_", "a");
            if (directMethod == null) {
                // Try finding a static method that takes List
                for (Method m : holderSetClass.getMethods()) {
                    if (java.lang.reflect.Modifier.isStatic(m.getModifiers()) &&
                        m.getParameterCount() == 1 &&
                        m.getParameterTypes()[0].isAssignableFrom(List.class)) {
                        directMethod = m;
                        break;
                    }
                }
            ...
            if (directMethod == null) {
                plugin.getConfigManager().debug("Could not find HolderSet.direct method");
                return null;
            }
            Object holderSet = directMethod.invoke(null, holderList);
            
            int radiusChunks = maxRadius / 16;
            
            // Try locate methods - multiple possible names for different mappings
            String[] findMethodNames = {
                "findClosestStructure",  // Mojang
                "method_43753",          // Fabric intermediary
                "m_220361_",             // Forge SRG
                "locateNearestMapStructure",  // Alternative
                "a"                      // Obfuscated
            };
            
            for (String methodName : findMethodNames) {
                try {
                    Method findMethod = cachedServerLevel.getClass().getMethod(methodName,
                        holderSetClass, blockPosClass, int.class, boolean.class);
                    Object pairResult = findMethod.invoke(cachedServerLevel, holderSet, blockPos, radiusChunks, false);
                    
                    if (pairResult != null) {
                        return extractLocationFromPair(pairResult, blockPosClass, world);
                    }
                } catch (NoSuchMethodException e) {
                    // Try next name
                }
            ...
            // If direct method not found, search by signature
            for (Method m : cachedServerLevel.getClass().getMethods()) {
                if (m.getParameterCount() == 4) {
                    Class<?>[] params = m.getParameterTypes();
                    if (holderSetClass.isAssignableFrom(params[0]) &&
                        blockPosClass.isAssignableFrom(params[1]) &&
                        params[2] == int.class &&
                        params[3] == boolean.class) {
                        try {
                            Object pairResult = m.invoke(cachedServerLevel, holderSet, blockPos, radiusChunks, false);
                            if (pairResult != null) {
                                plugin.getConfigManager().debug("Found structure via method: " + m.getName());
                                return extractLocationFromPair(pairResult, blockPosClass, world);
                            }
                        } catch (Exception e) {
                            // Try next
            ...
            plugin.getConfigManager().debug("Could not find locate method on ServerLevel");
            
        } catch (Exception e) {
            plugin.getConfigManager().debug("NMS locate failed: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private Location extractLocationFromPair(Object pairResult, Class<?> blockPosClass, World world) {
        try {
            // Try multiple names for getFirst (Pair class)
            Method getFirstMethod = findMethod(pairResult.getClass(), "getFirst", "method_15442", "m_124833_");
            if (getFirstMethod == null) return null;
            
            Object foundPos = getFirstMethod.invoke(pairResult);
            if (foundPos == null) return null;
            
            Method getXMethod = findMethod(blockPosClass, "getX", "method_10263", "m_123341_");
            Method getZMethod = findMethod(blockPosClass, "getZ", "method_10260", "m_123343_");
            
            if (getXMethod == null || getZMethod == null) return null;
            
            int x = (int) getXMethod.invoke(foundPos);
            int z = (int) getZMethod.invoke(foundPos);
